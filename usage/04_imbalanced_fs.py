...
train, test = chainer.datasets.split_dataset_random(
    dataset, int(len(dataset) * 0.90), seed=1
)

train_iter = chainer.iterators.SerialIterator(train, 800)
...
trainer = chainer.training.Trainer(updater, (2000, 'epoch'), out=result_dst_path)
